...
router.post('/', verifyAuth, async (req, res) => {
  try {
    ...
    // Validation checks
    if (!paymentMethod || !items || !shippingAddress || totalAmount === undefined) {
      return res.status(400).json({
        success: false,
        error: 'Missing required fields',
        requiredFields: ['paymentMethod', 'items', 'shippingAddress', 'totalAmount']
      });
    }
    ...
    items.forEach((item, index) => {
      ...
      // Check if variantIndex and weightIndex are provided for quantity update
      if (item.variantIndex === undefined || item.weightIndex === undefined) {
        itemErrors.push(`Item ${index + 1} is missing variantIndex or weightIndex for inventory update`);
      }
    });
    ...
    // Address validation
    const addressFields = ['name', 'street', 'city', 'state', 'postalCode', 'phone', 'email'];
    ...
    for (const item of items) {
      try {
        ...
        // Store the update operation for later execution
    ...
    // If there are any stock errors, return them before creating the order
